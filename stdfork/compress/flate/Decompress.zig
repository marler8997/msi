...
/// Initialize a new decompressor while preserving the window from a previous decompression.
/// This is needed for MSZIP format where each block is independently compressed but may
/// reference back into the previous block's decompressed data.
///
/// `window_size` should be the amount of valid decompressed data in the buffer from
/// the previous block (typically the size of the previous block's output, up to 32KB).
pub fn initPreservingWindow(input: *Reader, container: Container, buffer: []u8, window_size: usize) Decompress {
    assert(buffer.len >= flate.max_window_len);
    assert(window_size <= flate.max_window_len);

    // For MSZIP: if the previous block's output was larger than the window size,
    // we need to move the last 32KB to the beginning of the buffer
    const preserved_size = @min(window_size, flate.history_len);
    if (window_size > flate.history_len) {
        const src_offset = window_size - flate.history_len;
        vers.memmove(buffer[0..preserved_size], buffer[src_offset..window_size]);
    }
    ...
    return .{
        .reader = .{
            ...
            .buffer = buffer,
            .seek = preserved_size, // Start writing after the preserved window
            .end = preserved_size, // Buffer contains preserved_size bytes
        },
        .input = input,
        .next_bits = 0,
        .remaining_bits = 0,
...
fn rebase(r: *Reader, capacity: usize) void {
    ...
    vers.memmove(r.buffer[0..keep.len], keep);
    r.end = keep.len;
    r.seek -= discard_n;
}
